...
if (!isset($_SESSION['customer_id'])) {
    echo "<script>alert('Bạn cần đăng nhập để chỉnh sửa thông tin.'); window.location.href = 'login.php';</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql1 = "UPDATE customers SET full_name='$full_name', address='$address', dob='$dob' WHERE customer_id = $customer_id";
    $sql2 = "UPDATE users SET email='$email', phone='$phone' WHERE user_id = $customer_id";

    if (mysqli_query($conn, $sql1) && mysqli_query($conn, $sql2)) {
        // Hiển thị thông báo thành công và chuyển hướng về user.php
        echo "<script>alert('Cập nhật thành công!'); window.location.href = 'user.php';</script>";
        exit;
    } else {
        // Hiển thị thông báo lỗi
        echo "<script>alert('Lỗi khi cập nhật: " . mysqli_error($conn) . "');</script>";
        exit;
    }
}

// Lấy thông tin hiện tại
$sql = "SELECT u.username, u.email, u.phone, c.full_name, c.address, c.dob
        FROM users u
...
<body>
    ...
    <div class="container">
        <h2 class="container__heading">Chỉnh sửa thông tin tài khoản</h2>
